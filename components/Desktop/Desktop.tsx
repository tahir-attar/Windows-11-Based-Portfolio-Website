...
import { getBingWallpaper } from '../../utils/getBingWallpaper';
...
const Desktop = (): JSX.Element => {
  ...
  const [bingWallpaper, setBingWallpaper] = useState<string | null>(null);

  useEffect(() => {
    let isMounted = true;
    const fetchWallpaper = async () => {
      const url = await getBingWallpaper();
      if (isMounted && url) setBingWallpaper(url);
    };
    fetchWallpaper();
    return () => {
      isMounted = false;
